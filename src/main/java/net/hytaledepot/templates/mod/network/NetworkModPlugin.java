...
public final class NetworkModPlugin extends JavaPlugin {
  ...
  private final NetworkModTemplate service = new NetworkModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-network-mod-worker");
  ...
  public NetworkModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new NetworkModStatusCommand());
    getCommandRegistry().registerCommand(new NetworkModDemoCommand());
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[NetworkMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[NetworkMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class NetworkModStatusCommand extends CommandBase {
  ...
  private final class NetworkModDemoCommand extends CommandBase {
